...
```{r}
#| label: observations-names-dn-lq-ak
#| eval: true
...
**Answer:**
The mean distance between Denny's and La Quinta locations in Alaska is 4.41 miles.

```{r}
#| label: dn-lq-ak-distribution
#| eval: true
...
**Answer:**
The mean distance between Denny's and La Quinta locations in North Carolina is 65.4 miles.
...
**Answer:**
The mean distance between Denny's and La Quinta locations in Texas is 5.79 miles.
...
**Answer:**
The mean distance between Denny's and La Quinta locations in Oregon is 71.7 miles.
...
dn_lq_or <- full_join(dn_or, lq_or, by = "state")
dn_lq_or

dn_lq_or <- dn_lq_or %>%
  mutate(distance = haversine(longitude.x, latitude.x, longitude.y, latitude.y, round=3))

dn_lq_or_mindist <- dn_lq_or %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))

ggplot(dn_lq_or_mindist, aes(x=closest)) +
  geom_histogram()

dn_lq_or_mindist %>% summarise(mean_dist = mean(closest))
...
**Answer:**
The State where Mitch Hedberg's joke about La Quinte meaning beside Denny's is the most true in Alaska where the mean distance between locations is only 4.41 miles. Texas is not far behind at 5.79 miles. The joke does not appear to be true in either North Carolina or Oregon which have a mean distance of 65.4 and 71.7 miles between their Denny's and La Quinta locations.
